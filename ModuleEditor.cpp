...
update_status ModuleEditor::Update()
{
	if (showConfiguration)
	{
		...
		{
			const char* items[] = {"Backer House", "Model 2", "Model 3" };
			if (ImGui::Combo("Models", &currentItemSelected, items, IM_ARRAYSIZE(items)))
			{
				if (App->modelLoader->modelRendered != currentItemSelected)
				{
					...
					//App->grid->draw = currentItemSelected == 0 ? true : false; COMMENT
					App->modelLoader->ChooseModel(currentItemSelected);
				}
			}
		}
	...
	if (showAbout)
	{
		...
		ImGui::Text("Name: My Engine");
		ImGui::Text("Description: UPC School game developer master");
		ImGui::Text("Author: Gabriel Cambronero");
		ImGui::Separator();
